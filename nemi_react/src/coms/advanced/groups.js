...
import {GetGroupDisList, GroupEnable, GroupDestroy} from '../../apis/group'
...
class GroupDisabled extends Component {
  ...
  enabled_group = (e, item) =>{
    e.preventDefault();
    GroupEnable(this, item.id);
    this.flash_list()
  }

  destroy_group = (e, item) =>{
    e.preventDefault();
    GroupDestroy(this, item.id);
    this.flash_list()
  }
  ...
  render() {
    ...
    const columns = [{
      ...
      render: (text, record, index) => <div>
        <Button type="primary" size="small" onClick={(e)=>this.enabled_group(e, record)}>恢复</Button>
        <span> </span>
        <Button type="danger" ghost size="small" onClick={(e)=>this.destroy_group(e, record)}>彻底删除</Button>
      </div>
    }];
...
export default withRouter(GroupDisabled);
