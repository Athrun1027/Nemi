...
import { Input, Modal, Tabs, Popconfirm, Upload, Progress, Tree, Breadcrumb, Row, Col, Icon, Tooltip, Button, Table } from 'antd';
...
class SpaceGroup extends Component {
  constructor(props) {
    super(props);
    ...
    let root_folder_id = false;
    let folder_id = false;
    let groups_index = "0";
    if (group_cookie.groups.length !== 0){
      root_folder_id = group_cookie.groups[0].root_folder_id;
      folder_id = group_cookie.groups[0].root_folder_id;
      groups_index = group_cookie.groups[0].id;
    }
    this.state = {
        user: user_cookie,
        selectedRowKeys: [],
        ...
        groups: group_cookie.groups,
        groups_mine: group_cookie.groups_mine,
  ...
  GroupChange=(activeKey)=>{
    let root_folder_id = 0;
    for (var i = this.state.groups.length - 1; i >= 0; i--) {
      if(this.state.groups[i].id === activeKey){
        root_folder_id = this.state.groups[i].root_folder_id
      }
    }
    this.setState({
      groups_index: activeKey,
      root_folder_id: root_folder_id,
      folder_id: root_folder_id,
    })
    this.flash_list(root_folder_id)
  }
  ...
  clearAll = () => {
    ...
    this.flash_list(this.state.root_folder_id)
  }
  onInputChange = (e) => {
  ...
  flash_list=(id) =>{
    ...
    FolderTree(this, this.state.root_folder_id);
    NProgress.done();
  }
  ...
  render() {
    ...
    const columns = [{
      ...
      render: (text, record, index) => {
        ...
        if (record.open_time !== null) {
          let time = Date.parse(new Date(record.open_time)) - Date.parse(new Date())
          if (time > 0){
            return <Tooltip title={record.open_time+'开放下载'}>
              <Icon type="clock-circle" style={{ fontSize: 20}}/>
            </Tooltip>
          }
        }
    ...
    return (
      <div>
        <Location>
          <Row gutter={16} className="borad-header group-tab">
            <Col span={17}>
              <h3>群组空间</h3>
              <span>默认加入的群个数有限制，如溢出，请自行联系管理员扩容</span>
              <Tabs activeKey={this.state.groups_index} onChange={this.GroupChange}>
                {this.state.groups.map((item)=>
                  <Tabs.TabPane tab={item.nickname} key={item.id} />)}
              </Tabs>
            </Col>
            <Col span={7}>
              <div className="gutter-box">
                <div className="borad-header-inline">
                  ...
                  <Tooltip title="这是你所在的群组数" placement="bottom" >
                    <h1>{this.state.groups.length}<span className="text-gay text-small"> /10</span></h1>
                  </Tooltip>
                </div>
                <div className="borad-header-inline">
                  <span className="text-gay">管理的群</span><br/>
                  <Tooltip title="很抱歉，创建群组是有上限的" placement="bottom" >
                    <h1>{this.state.groups_mine.length}<span className="text-gay text-small"> /5</span></h1>
                  </Tooltip>
                </div>
              </div>
            </Col>
        ...
        <div className="space-body">
          ...
          >
            ...
            <Button
              type="primary"
              onClick={(e)=>{
                const {groups_mine, groups_index} = this.state;
                let bucket_id = 0;
                for (var i = groups_mine.length - 1; i >= 0; i--) {
                  if (groups_mine[i].id === groups_index) {
                    bucket_id = groups_mine[i].bucket_id
                  }
                }
                FileUpload(this, bucket_id)
              }}
...
export default withRouter(SpaceGroup);
