...
class ShareFrom extends Component {
  ...
  render() {
    ...
    const columns = [{
      ...
      key: 'own',
      render: (text, record, index) => {
        return <a >{record.own.own_username+'('+record.own.own_nickname+')'}</a>
      }
    },{
      title: '下载',
      key: 'download',
      render: (text, record, index) => {
        ...
        return <a href={NEMI_URL+"files/download/" + record.original_id}><Icon type="cloud-download-o" style={{ fontSize: 20}}/></a>
      }
    },{
      title: '大小',
      ...
      render: (text, record, index) => <div>
        <Popconfirm placement="topRight" title="╥﹏╥…你竟然要分享" onConfirm={(e)=>this.disabled_resource(e, record)} okText="取消" cancelText="算了">
          <Button type="danger" size="small">取消分享</Button>
        </Popconfirm>
      </div>
    }];
    ...
    return (
      ...
      <Location>
          <Row gutter={16} className="borad-header">
            <Col span={17}>
              <h3>分享</h3>
              <span>这些是你分享出去的文件</span>
            </Col>
            <Col span={7}>
              <div className="gutter-box">
                <div className="borad-header-inline">
                  <span className="text-gay">资源总数</span><br/>
                  <Tooltip title="嫌少？那就去分享啊" placement="bottom" >
                    <h2>{this.state.data_share_from.length}<span className="text-gay text-small"> /1000</span></h2>
                  </Tooltip>
                </div>
              </div>
            </Col>
        ...
        <div className="space-body">
          ...
          dataSource={this.state.data_share_from} />
        </div>
      </div>
    );
  }
...
export default withRouter(ShareFrom);
