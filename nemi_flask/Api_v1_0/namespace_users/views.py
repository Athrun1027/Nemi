...
from .api_init import api, come_in_user_class
    ...
    def create_instance(self, body_data):
        ...
        *.将password字段加密
        ...
        if "img_url" not in body_data:
            body_data["img_url"] = "https://gw.alipayobjects.com/zos/rmsportal/BiazfanxmamNRoxxVxka.png"
        # 判断该模型是否支持所有输入属性
...
@api.route('/groups/<int:pk>')
@api.param('pk', '需要查找的用户ID')
@api.response(400, '参数形式有误')
class UserGroupList(ResourceItem):
    model = User
    db = db
    ...
    def get(self, pk):
        ...
        groups_data = []
        groups_mine_data = []
        for item in instance.groups:
            if item.is_enable is False:
                continue
            groups_data.append(
                {
                    "id": str(item.id),
                    ...
                    "root_folder_id": item.space_mine[0].root_folder_id,
                    "group_size": len(item.users)
                }
            )
        for item in instance.groups_mine:
            if item.is_enable is False:
                continue
            users = []
            ...
            groups_mine_data.append(
                {
                    ...
                    "join_time": str(item.join_time),
                    "root_folder_id": item.space_mine[0].root_folder_id,
                    "bucket_id": item.space_mine[0].bucket_kids[0].id,
                    "group_size": len(item.users),
                    "users": users
                }
            )
        return {"groups": groups_data, "groups_mine": groups_mine_data}


@api.route('/<int:pk>/class/')
@api.param('pk', '需要查找的用户ID')
@api.response(400, '参数形式有误')
class ChangeClass(ResourceEdit):
    model = User
    db = db

    @api.marshal_with(come_out_user_item_success, code=200, description="成功修改用户班级信息!")
    @api.expect(come_in_user_class)
    @login_require
    def put(self, pk):
        """修改用户班级信息"""
        return super(ChangeClass, self).put(pk)
